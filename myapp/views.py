...
def moviepage(request, movieid, viewerid):
    if request.user.is_authenticated and viewerid == request.user.id:
        ...
        revs = ViewerMovie.objects.select_related('viewer').filter(movie_id=movieid, review__isnull=False).order_by(
            '-rtime')
        top3_reviews = []
        count = 0
        for re in revs:
...
def watched(request, viewerid):
    if request.user.is_authenticated and viewerid == request.user.id:
        ...
        for item in row:
            ...
            rating = item[7]
            movies.append(
                {'id': id, 'title': title, 'genre': genre, 'plot': plot[:100] + '...', 'lang': lang, 'poster': poster,
                 'rating': rating})
